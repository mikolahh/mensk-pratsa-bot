...
{   
  ...
  private $web_app_url = 'https://bot.mikalay.tech/bot/users';
          ...
          if ($this->update_obj->updateType() == 'private') {
            ...
            if(!$this->action_data['access_isset'] && ($this->action_data['user_id'] != $this->group_admin_id)) {              
              try {
                ...
                $add_text = "Ошибка удаления сообщения(message_id: $message_id) пользователя((user_id: $from_user_id, user_name: $from_user_name)) без доступа из группы";              
                $this->actionErrorLogDev($e, $add_text);                
              } 
  ...
  {    
    ...
    $user_isset = !empty($this->builder_users->where(['user_id' => $user_id])->countAllResults());         
    if ($user_isset) {
  ...
  {       
    ...
    if ($user_id == $this->group_admin_id) {
      ...
      if ($access_isset) {
        ...
        $text_access_first = "<b>Теперь доступ к размещению объявлений в группе условно платный.</b>" . PHP_EOL . "Плата берется с целью поддержания порядка в чате и  защиты от мошенничества." . PHP_EOL . "<i>Если вы оплатили доступ и при этом шлете объявления сомнительного характера (поиск курьеров для торговли наркотиками или откровенный скам (связки, арбитраж), то ваш аккаунт блокируется без возврата денежных средств.</i>" . PHP_EOL . "Стоимость доступа:" . PHP_EOL . "<b>1 год: 3 рубля.</b>" . PHP_EOL . "Плата символическая, но позволит избежать многих проблем и как-то поддерживать проект.";               
  ...
  private function actionPayConfStart()
  {
    $user_id = $this->action_data['user_id'];
    $group_admin_link = $this->group_admin_link;
    $text = "<b>Отправьте квитанцию ЕРИП об оплате в виде текста или pdf-файла</b>" . PHP_EOL . "Доступ к показу объявлений будет дан сразу, а затем подтвержден после проверки платежа"  . PHP_EOL . "При возникновении затруднений просто отправьте произвольный текст, и с вами свяжется администратор группы: $group_admin_link";
  ...
  {    
    ...
    $date_pay_sec_obj = new \DateTime('1year', $this->date_time_zone_obj);
  ...
  private function actionUserForPayConfFin()
  {
    ...
    $text = "Результат обработки платежа для пользователя:" . PHP_EOL . "Имя: $user_name_for_admin" . PHP_EOL . "Ссылка: $user_link_for_admin" . PHP_EOL . "Дата окончания платежа: <b>$date_pay</b>" . PHP_EOL . "Временный допуск отменен" . PHP_EOL . "Данные о платеже удалены из базы данных";
  ...
  private function handleUserAdt()
  {
    $access_isset = $this->action_data['access_isset']; 
    if ($access_isset) {
  ...
  {     
    $date_current_obj = new \DateTime('now', $this->date_time_zone_obj);       
    $bot_messages_id = $this->builder_bot_group_adts->limit(10)->get()->getResultArray();   
    $deleted_messages = [];    
